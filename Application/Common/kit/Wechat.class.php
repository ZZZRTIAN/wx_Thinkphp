...
class Wechat{
	...
	const MSGTYPE_EVENT_SUBSCRIBE = 'subscribe';
	const MSGTYPE_EVENT_UNSUBSCRIBE = 'unsubscribe';
	...
	const TAR_CREATE = '/tags/create?';
	const TAR_GET = '/tags/get?';
	const TAR_UPDATE = '/tags/update?';
	const TAR_DELETE = '/tags/delete?';

	const TAG_USER_GET = '/tag/get?';

	const TAR_MEMBER_ADD = '/tags/members/batchtagging?';
	const TAR_MEMBER_DELETE = '/tags/members/batchuntagging?';

	const GET_TOKEN_URL = '';

	const KF_PREFIX = 'https://api.weixin.qq.com/customservice/kfaccount';
	const KF_ADD = '/add?';
	const KF_UPDATE = '/update?';
	const KF_DEL = '/del?';
	const KF_GET = '/getkflist?';

	const MEDIA_LIST = '/material/batchget_material?';
	const MEDIA_ADD = '/material/add_material?';
	const MEDIA_GET = '/material/get_material?';
	...
	public function get_access_token() {
		...
		$access_token_json = file_get_contents($file_name);
		...
		if (empty($access_token_json)) {
			...
			if (empty($result)) {
				...
				$result['expires_time'] = time() + $result['expires_in'];//计算有效时长
			...
			if ($access_token_arr['expires_time'] - 3600 < time() ) { //重新获取
				file_put_contents('abc.log', 'qwe');
				$result = $this->http_get(self::API_URL_PREFIX . self::AUTH_URL . "appid={$this->appid}&secret={$this->appsecret}");
				if (empty($result)) {
					return false;
				}
	...
	public function sendGroupMassMessage($data){
		...
		file_put_contents('res.log', json_encode($result));
		if ($result){
			$json = json_decode($result,true);
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
	...
	public function getMediaByType($param)
	{
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::API_URL_PREFIX . self::MEDIA_LIST . 'access_token=' . $this->access_token, json_encode($param));
		if ($result) {
			$json = json_decode($result,true);
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
	...
	public function addMaterial($fileinfo, $type)
	{
		if (!$this->access_token && !$this->checkAuth()) return false;
		$real_path="{$_SERVER['DOCUMENT_ROOT']}{$fileinfo['filename']}";
		$url = self::API_URL_PREFIX . self::MEDIA_ADD . 'access_token=' . $this->access_token. '&type='.$type;
		$data = array('media'=>new CURLFile(realpath($real_path)), 'filename' => $fileinfo['filename'], 'content-type' => $fileinfo['content-type']
	 ...
	 * @return bool
	 */
	public function getMaterial($param, $newfilename)
	{
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::API_URL_PREFIX . self::MEDIA_GET . 'access_token='. $this->access_token, json_encode($param));
	 ...
	 * 获取视频素材
	 */
	public function getVideoMaterial($param) {
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::API_URL_PREFIX . self::MEDIA_GET . 'access_token='. $this->access_token, json_encode($param));
	...
	{
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_get(self::API_URL_PREFIX . self::TAR_GET . 'access_token='. $this->access_token);
		return json_decode($result, true);
	}
	...
	{
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::API_URL_PREFIX . self::TAR_CREATE . 'access_token='. $this->access_token, json_encode($param));
		return json_decode($result, true);
	}
	...
	{
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::API_URL_PREFIX . self::TAR_UPDATE . 'access_token='. $this->access_token, json_encode($param));
		return json_decode($result, true);
	}
	...
	{
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::API_URL_PREFIX . self::TAR_DELETE . 'access_token='. $this->access_token, json_encode($param));
		return json_decode($result, true);
	}
	...
	{
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::API_URL_PREFIX . self::TAR_MEMBER_ADD . 'access_token='. $this->access_token, json_encode($param));
		return json_decode($result, true);
	}
	 ...
	 * 为用户取消标签
	 */
	public function deleteUserTag($param)
	{
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::API_URL_PREFIX . self::TAR_MEMBER_DELETE . 'access_token='. $this->access_token, json_encode($param));
		return json_decode($result, true);
	}
	 ...
	 * 获取标签下粉丝列表
	 */
	public function getTagUserList($param)
	{
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::API_URL_PREFIX . self::TAG_USER_GET . 'access_token='. $this->access_token, json_encode($param));
		return json_decode($result, true);
	}
